...
const Threads = ({ color = [1, 1, 1], amplitude = 1, distance = 0, enableMouseInteraction = false, ...rest }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!gl) {
        console.error('❌ WebGL not supported. Please enable WebGL in your browser.');
        console.log('📖 Instructions: Visit chrome://flags or about:config to enable WebGL');
        return;
      }
      
      console.log('✅ WebGL detected:', gl.getParameter(gl.VERSION));
    } catch (e) {
    ...
    try {
      ...
      if (!glContext) {
        console.error('❌ Unable to create WebGL context from Renderer');
        return;
      }
      ...
      const program = new Program(glContext, {
        ...
        uniforms: {
          iTime: { value: 0 },
          iResolution: {
            value: new Color(glContext.canvas.width, glContext.canvas.height, glContext.canvas.width / glContext.canvas.height)
          },
      ...
      return () => {
        ...
        if (rendererRef.current && rendererRef.current.gl) {
          ...
          if (glContext.canvas && container.contains(glContext.canvas)) {
